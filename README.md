...
Прежде всего, вам понадобится база данных Postgres. Запустите её на стандартном порту `127.0.0.1:5432`, либо укажите другой адрес через переменную окружения `DATABASE_URL`.
...
- `ROLLBAR_ACCESS_TOKEN` — токен к Rollbar, можно оставить незаполненным, но тогда отчёты об ошибках не будут долетать до Rollbar.
- `ROLLBAR_ENVIRONMENT` — название окружения, где развёрнут сайт. Возможны любые значения, но рекомендуется указывать имя хоста, например `star-burger-pallada` для сервера или `pelid80-dev` для разработческой версии сайта. Так вы избежите путаницы, когда похожие ошибки прилетают сразу с нескольких хостов. По умолчанию значение выставлено в `development`.
- `DATABASE_URL` — адрес базы данных. По умолчанию, сайт подключается в БД на порту `127.0.0.1:5432`.
